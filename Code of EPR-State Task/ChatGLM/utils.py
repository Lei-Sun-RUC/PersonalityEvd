...
"""
import sys
sys.path.append('/data6/sl/EMNLP_pers_evidence/experiment/state_experiments')
...
class GLM3PromptDataSet(Dataset):
    def __init__(self, data_info, tokenizer, max_len, max_src_len, is_skip):
        self.all_data = []
        skip_data_number = 0
        """
        with open(data_path, "r", encoding="utf-8") as fh:
            for i, line in enumerate(fh):
                ...
                src_tokens = [tokenizer.get_command("<|user|>")] + tokenizer.encode("\n", add_special_tokens=False) + \
                             tokenizer.encode(sample["input"], add_special_tokens=False)
        ...
        data_formatter=DataFormatter(data_info["dlg_path"],data_info["state_anno_path"],data_info["data_type"],data_info["train_ratio"])
        train_data=data_formatter.convert2chatglm3()
