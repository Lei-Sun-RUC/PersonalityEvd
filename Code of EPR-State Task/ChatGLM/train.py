...
def parse_args():
    ...
    parser.add_argument("--train_ratio", type=float, default=1, help="the train set ratio to be used")
...
def main():
    ...
    # load data
    data_info = {
            "dlg_path":args.dlg_path,
            "state_anno_path":args.state_anno_path,
            "data_type":args.data_type,
            "train_ratio":args.train_ratio}
    
    train_dataset = MODE[args.mode]["dataset"](data_info, tokenizer, args.max_len, args.max_src_len, args.is_skip)
    if args.local_rank == -1:
    ...
    for epoch in range(args.num_train_epochs):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.save_model_step is not None and global_step % args.save_model_step == 0:
                    ...
                    if ds_config["zero_optimization"]["stage"] == 3:
                        ...
                        if args.global_rank <= 0:
                            save_model(model, tokenizer, args.output_dir, f"epoch-{epoch + 1}-global_step-{global_step}",
                                       state_dict)
                    else:
                        if args.global_rank <= 0:
                            save_model(model, tokenizer, args.output_dir, f"epoch-{epoch + 1}-global_step-{global_step}")
        ...
        if ds_config["zero_optimization"]["stage"] == 3:
            state_dict = model._zero3_consolidated_16bit_state_dict()
            if args.global_rank <= 0:
                save_model(model, tokenizer, args.output_dir, f"epoch-{epoch + 1}-global_step-{global_step}", state_dict)
        else:
            if args.global_rank <= 0:
                save_model(model, tokenizer, args.output_dir, f"epoch-{epoch + 1}-global_step-{global_step}")
